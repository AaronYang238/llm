...
> 已就位（类型 B 横向对比章，AI Infra 平台层扩展第一章）：[chapters/13-集群编排与调度.md](chapters/13-集群编排与调度.md)，主图 [svg/31-cluster-orchestration.svg](svg/31-cluster-orchestration.svg)（作业 → 调度器三职责 → 物理卡池）。以 K8s 为主深读，Slurm / Ray 对照。

- [x] **全景图与共同骨架**：作业 → 调度器（排队/配额、gang+拓扑放置、隔离/共享三职责）→ 物理卡池；gang 为什么是 GPU 作业的分水岭（13 §13.2，主图 `svg/31-cluster-orchestration.svg`）
- [x] **Kubernetes（深读）**：device plugin / GPU Operator、Volcano/Kueue gang 调度、DRA 拓扑、HPA/KEDA 弹性（13 §13.3）
- [x] **Slurm（对照）**：HPC 批队列，gang / 拓扑 / 高性能网络原生；与 K8s 镜像对称（13 §13.4）
- [x] **Ray（对照）**：应用层分布式 Python，task/actor + placement group，RLHF 底座（13 §13.5）
- [x] **GPU 共享与切分**：MIG / MPS / time-slicing 隔离强度递减（13 §13.6）
- [x] **弹性、抢占与拓扑**：KEDA + scale-to-zero、优先级抢占、bin-pack vs spread（13 §13.7）
- [x] **对比矩阵 + 选型清单**：K8s/Slurm/Ray × 12 维度；按负载重心选底座（13 §13.8–§13.9）
