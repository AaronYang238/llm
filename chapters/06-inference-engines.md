...
4. **TensorRT-LLM 改 batch 上限要重 build**:engine 编译期固化了 max\_batch/max\_seq。线上要扩容量得重新 build + 重启,不能热改——这是它灵活性差的具体体现。
...
1. **（概念）** "所有推理引擎本质都是同一个 loop"——这个 loop 每个 step 做哪几件事？各对应前面哪些阶段的知识？
2. **（选型）** 三个场景各选哪个引擎、为什么：(a) 通用大规模 serving；(b) DeepSeek-V3 部署；(c) 笔记本上跑 7B 做隐私推理。
3. **（应用）** 你要把一个 vLLM 还没支持的新模型接进去，大致要做哪几步？哪个文件是"母本"可以照着改？
4. **（判读）** 你的 benchmark 吞吐和官方数字对不上。在下结论"引擎有问题"之前，应该先确认什么？
5. **（概念）** SGLang 在 DeepSeek-V3 上为什么用 "Attention DP + FFN EP" 这种非对称并行？根子在哪个结构决策？

<br>

**参考答案**

1. ① Scheduler 选请求（阶段 5）→ ② KVCache 分配 block / 查 prefix cache（阶段 5）→ ③ Executor 驱动 Worker 跑 forward（阶段 2/3/4）→ ④ Sampler 出 token（阶段 1）→ ⑤ 回写 KV、输出。（§6.2.1）
2. (a) **vLLM**（全场景均衡、生态最广，默认 baseline）；(b) **SGLang**（DP attention + EP + DeepEP 全栈最成熟）；(c) **llama.cpp / Ollama**（CPU/消费硬件、本地隐私，数据不出本地）。（§6.9）
3. 写模型类（`nn.Linear` 换成 `QKVParallelLinear` 等 TP-aware 版本）→ 实现 `load_weights` 映射 HF 权重 → 在 `registry.py` 注册。母本是 `model_executor/models/llama.py`，照着 diff 改。（§6.3.3）
4. 先确认在**自己的硬件 + 真实负载**上测——官方数字是特定硬件/并发/seq 下的最优，脱离上下文的吞吐数没意义。（§6.10 第 2 条）
5. 因为 **MLA 的 KV 太小，不适合 TP 切**（切不动还引入通信），所以 attention 走 DP（每卡持完整 KV）；而 256 个专家必须分散 → FFN 走 EP。根子是 **MLA 这个结构决策**。（§6.4.3，详见阶段 9 §9.8.1）

> 第 2、3 题是工程现场最高频的两类需求（选型、接模型）——能脱口而出，说明这章读透了。
