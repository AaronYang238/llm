...
FlashMLA 是 DeepSeek 2024 年开源的 attention kernel——**专为 MLA（Multi-head Latent Attention）设计、专为 Hopper 优化**。MLA 把 KV cache 压到标准 MHA 的 ~1/16，但要在 H100 上跑出竞争力，必须重写一个 kernel——FlashAttention 假设 QKV 同秩，MLA 的低秩潜变量不适配。
...
**收益**：attention kernel 直接在 `d_latent=512` 维度上跑，**根本不需要 materialize per-head 的 K/V**。HBM 流量降到 MHA 的 ~1/16，与 cache 容量节省一致。
