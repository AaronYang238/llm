...
**(1) Online softmax：分块也能算对 softmax**

朴素 softmax 要先求全局 max（避免 $e^x$ 溢出）再 exp 再求 sum，**必须一次扫完整行**——这正是 tiling 的拦路虎：分块时还没见到后面的 K，怎么算前面的 softmax？Online softmax 把"两遍扫"改成"一遍流式扫 + 边走边修正"，给出**增量公式**——每来一块新分数 $x_{new}$，更新三个 running 量：
...
**三个量分别是什么**（这是看懂 FA kernel 的关键，每个 Q 行各维护一份）：

| 量 | 是什么 | 角色 |
|---|---|---|
| $m$ | running max（运行最大值） | **数值稳定锚**——所有 $e^{\cdot}$ 都相对 $m$ 算，保证不溢出 |
| $\ell$ | running denominator（运行分母） | softmax 的分母 $\sum e^{x-m}$，最后要除的那一项 |
| $O$ | running **未归一化**输出 | $\sum e^{x-m}\cdot V$，**注意还没除 $\ell$** |

**为什么这套增量是精确等价的**：难点在 max 会变——新块可能带来更大的值，于是 $m$ 从 $m_{old}$ 更新到 $m_{new}$。但**之前累加进 $\ell$、$O$ 的项都是按旧 max 算的 $e^{x-m_{old}}$**，基准变了就对不齐。修正办法就是那个 $e^{m_{old}-m_{new}}$ 因子：把旧的 $\ell$、$O$ 整体**乘上它、重新刻度到新 max 基准**，再加新块的贡献。所以 online softmax 和朴素 softmax 在数学上**逐位相等**，不是近似。

**最后别忘了归一化**——`O` 全程是"未归一化"的分子，处理完一个 Q 行的所有 K/V 块后，要**整行除一次 $\ell$** 才是真正的 attention 输出：

$$O_{\text{final}} = O_{acc} / \ell$$

这一步只在写回 HBM 前做一次（下面伪代码里很多简化版会漏写它，是常见误读点）。把"加权 V"和"归一化"拆开、加权边走边累加、归一化留到最后——这正是 tiling 能成立的全部秘密。

**(2) IO-aware tiling：把 attention 装进 SMEM**
...
**省的到底是什么——一个常见误解**：FlashAttention **不是"少 load 了 K/V"**。恰恰相反，外循环每个 Q tile 都要把全部 K/V 在内循环里重扫一遍——K/V 实际被**重复 load 了 $S/B_r$ 次**。它真正干掉的是**两次 $O(S^2)$ 的中间矩阵读写**：

| 张量 | 朴素 attention 的 HBM 流量 | FlashAttention |
|---|---|---|
| Q / K / V | $O(S\cdot d)$ | K/V 被重 load → $O(S^2 d / B_r)$ |
| **$S=QK^\top$**（分数矩阵） | **写 $S^2$ + 读 $S^2$** | **0（全在 SMEM）** |
...
- **HBM 流量主项从 $O(S^2)$ 降到 $O(S \cdot d)$**——长 seq 时 $S \gg d$，节省巨大；
- **不 materialize $[S, S]$ 矩阵**——显存从 $O(S^2)$ 降到 $O(S)$，这是长上下文不 OOM 的关键；
...
PagedAttention 的思路**直接搬操作系统的虚拟内存**：进程看到连续的虚拟地址，但物理内存按固定大小的页（page）离散分配，靠页表做映射。PagedAttention 一一对应地搬过来——KV cache 切成固定大小的"页"（block，典型 16 token），用 block table 维护"逻辑序列 → 物理 block"的映射：

| 操作系统虚拟内存 | PagedAttention |
|---|---|
| 虚拟地址 | 逻辑 token 位置 $t$ |
| 物理页框 (page frame) | 物理 KV block（固定 16 token） |
| 页表 (page table) | block table |
| 按需分页 (demand paging) | 按需分配 block |
| 换出到磁盘 (swap) | KV block 换出到 CPU / NVMe（§5.8） |
| 写时复制 (copy-on-write) | prefix sharing（前缀共享） |

**为什么"固定大小的块"能同时消掉两类碎片**（这是整个设计的题眼）：

- **外部碎片**来自"大小不一的空洞拼不起来"。固定大小后，**每个空闲 block 都一样大、完全可互换**——任何新请求的任何一页都能填进任意空闲 block，空洞永远能被利用 → 外部碎片消失。
- **内部碎片**来自"按 `max_seq_len` 预分配的巨大浪费"。改成"按需一页页加、每页只 16 token"，最坏只浪费**最后一页里没填满的部分（< 16 token）** → 内部碎片从"max_seq 级（几千 token）"骤降到"< block_size 级（< 16 token）"。

两类碎片一起治掉，显存利用率从 20–40% 拉到 ~90%——**多出来的显存直接变成更大的并发 batch**，这才是吞吐翻倍的根（回阶段 0 §0.2.2：更大 batch 把 decode 的 GEMV 推过 ridge point）。
...
#### block_size 怎么选：一个四方权衡

`block_size` 是 PagedAttention 唯一的核心旋钮（vLLM 默认 16）。它不是越小越好——是四个力的平衡：

| 调小 block（如 8） | 调大 block（如 32 / 64） |
|---|---|
| ✅ 内部碎片更小（最后一页浪费 < block_size） | ❌ 内部碎片更大 |
| ✅ prefix 共享粒度更细（见下） | ❌ 前缀要对齐到更大边界才能共享 |
| ❌ block table 更长、lookup 次数更多 | ✅ table 更短、lookup 更少 |
| ❌ HBM 访问更碎，coalescing 差、带宽利用低 | ✅ 访存更连续，带宽利用高 |

关于**prefix 共享粒度**：两个请求只有在**前缀长度对齐到 block 边界**时才能共享同一物理 block——block=16 时，能共享的前缀只精确到 16 的整数倍那一段；block=64 则要 64 对齐，更粗。所以**小 block 利于细粒度前缀复用**（多轮对话、system prompt 共享场景尤其受益）。

16 是 vLLM 跑出来的 sweet spot：内部碎片 < 16 token 可接受、访存够连续不至于砸带宽、前缀共享够细。长上下文或特殊访存模式下偶尔调到 8 或 32，但绝大多数场景默认 16 不用动。
...
3. **CUDA Graph 不友好**：FA 的 launch 参数随 seq\_len 变，capture 后下次 seq\_len 不同就 replay 失败（回阶段 0 §0.3 的 CUDA Graph 限制）。
...
| GQA（H\_kv=8） | `2 × L × H_kv × d × 2 bytes` | 80 × 8 × 128 × 4 = **327 KB** |
...
**矩阵吸收的关键观察**：attention 内积 `Q @ (K_compressed @ W_K_up).T` 可改写成 `(Q @ W_K_up.T) @ K_compressed.T`。**把 W\_K\_up 吸收进 Q 的投影矩阵**——离线把 `W_Q_per_head` 与 `W_K_up.T` 预乘成 `W_Q'`，推理时：
...
**为什么这招合法又免费**：`Q @ (K_c @ W_K_up)^\top = (Q @ W_K_up^\top) @ K_c^\top` 纯粹是**矩阵乘法结合律**——只是换了结合顺序，结果**逐位相等、不是近似**。而 `W_Q' = W_Q @ W_K_up^\top` 可以**离线预乘好、存成一个权重**，推理时零额外开销。唯一代价是 `W_Q'` 的输出维从 head 维（128）变成 latent 维（512）略大——但它换掉的是"**每个 decode step 都把整份 KV up-project 回 head 维**"这个大头，净赚。

**收益**：attention kernel 直接在 `d_latent=512` 维度上跑，**根本不需要 materialize per-head 的 K/V**。HBM 流量降到 MHA 的 ~1/30，与 §4.5.1 表里的 cache 容量节省一致。

但 RoPE 那部分（DeepSeek-V3 设计是 `d_rope=64` per-head，与 d\_latent 解耦）必须保持 per-head 计算——所以 MLA kernel 的实际结构是**"低秩主路 + 小 RoPE 旁路"**：
...
### 4.5.4 MLA 的连锁反应：从 kernel 一直牵动到部署

MLA 的"**压缩 KV 跨所有 head 共享一份**"这一个设计，像多米诺一样牵动整条推理栈——理解这条链，才算真懂为什么 DeepSeek 部署和别的模型不一样：

1. **kernel 层（本节）**：KV 没有 head 维、是个 `d_latent=512` 的共享潜向量 → 标准 FlashAttention 跑不了 → 必须重写 FlashMLA。
2. **并行层（回阶段 2 §2.3.4）**：压缩 KV 是**所有 head 共享的一份**，没法像 GQA 那样"按 head 切给不同卡" → **MLA 不能做 TP-attention**。硬要 TP 就得把这份 latent 复制到每张卡，省 KV 的好处全吐回去。
3. **部署层（回阶段 9 §9.8）**：既然 attention 不能 TP，DeepSeek 推理就改用 **DP-attention**——每卡持完整 attention + 完整 KV、各算各的 batch，attention 路径**零通信**；跨卡通信只剩 MoE 的 all-to-all。

所以 §4.5.3 第 2 点"规避 `TP ≤ H_kv`"其实有另一面：**不是"不用受 TP 约束"，而是"根本不能 TP-attention，只能 DP-attention"**。一句话——**MLA 在 kernel 上省了 KV，代价是在并行上放弃了 TP-attention 换成 DP**。这是 kernel（本章）、并行（阶段 2）、部署（阶段 9）三章在同一个设计上的交汇点。

**一个 kernel 级的 co-design 细节**：吸收形式（absorb）在 **decode 最优**——只读 512 维压缩 cache，memory-bound 场景流量最小；但 **prefill 阶段**有时反而用**非吸收形式**（先把 K up-project 成 full head 维再算大 GEMM）更快——因为 prefill 是 compute-bound、吃得下大 matmul，而吸收形式在长 prompt 下的 latent 维大 matmul 未必划算。vLLM / SGLang 的 MLA backend 因此会**按 prefill / decode 切换吸收与否**——又一个"算法 ↔ kernel 按场景共设计"的实例（§4.0）。
...
| `tl.constexpr` | 编译期常量，让 BLOCK\_SIZE 等参与展开优化 |
...
| 优化 | 怎么做 |
|---|---|
| **autotuning** | `@triton.autotune(configs=[...], key=['hidden'])`，按 shape 自动选 BLOCK\_SIZE / `num_warps` / `num_stages` |
...
LLM 推理 / 训练里**最大的算力消耗是 GEMM**——QKV 投影、O 投影、FFN 三层 Linear、lm\_head，加起来超过 attention + 通信。**GEMM 跑得快不快直接决定模型整体吞吐**。本节讲两个生产环境的 GEMM 基础设施：CUTLASS（模板库，写新 kernel）和 cuBLASLt（NVIDIA 官方运行时库，调现成 kernel）。
...
|---|---|---|---|
| **Prefill GEMM** | (B·S) × N\_out × N\_in | 长 prompt 投影，B·S 可达 8192+ | 高 → compute-bound |
| **Decode GEMM** | B × N\_out × N\_in | 单 token，B 是并发请求数 | 低（B 小时）→ memory-bound |
...
CUTLASS 的核心价值是 **epilogue 可任意定制**——`LinearCombination` 是默认的 `αAB + βC`，你可以替换成"GEMM + bias + GELU + cast\_to\_fp8"等任意元素级链。**这是与"调 cuBLAS 黑盒"的本质差异**。
...
回阶段 2 §2.2.2：Tensor Parallel 的 attention 块结构是 column-parallel(QKV) → row-parallel(O)，FFN 块是 column-parallel(W\_gate, W\_up) → row-parallel(W\_down)。每个 row-parallel 输出后必须 AllReduce 把各 rank 部分和加起来：
...
- **`Dao-AILab/flash-attention` + `deepseek-ai/FlashMLA` GitHub** — 实际源码，对照 §4.2.4 / §4.5.5 的路径表读，~2 周能摸清架构。
