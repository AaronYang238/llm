...
> - **compute-bound（算力受限）**：算术强度 **高于** ridge point，瓶颈是**算力**——Tensor Core 已满载，数据早备好在等。典型：prefill 的大 **GEMM**（General Matrix Multiply，矩阵×矩阵乘 `[M,K]×[K,N]`）。优化方向是**提算力**（换更快 kernel、用 FP8、提 Tensor Core 利用率）。
> - **memory-bound（带宽受限）**：算术强度 **低于** ridge point，瓶颈是**显存带宽**——算力大量闲置，时间全花在从 HBM 搬数据上。典型：decode 的单 token **GEMV**（General Matrix-Vector，矩阵×向量乘，即 GEMM 中 `M=1` 的退化）、读 KV cache。优化方向是**减少访存**（融合算子、量化减字节、加大 batch 提复用）。
