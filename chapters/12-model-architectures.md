...
1. **（概念）** "现代 LLM 高度同质化"——它们共享的标准骨架是什么？真正的差异收敛在哪六个旋钮上？
2. **（config 判读）** 你拿到一个 `config.json`，里面有 `num_key_value_heads=8`、`num_experts=128`、`num_experts_per_tok=8`、`sliding_window=4096`。这个模型的 attention、FFN 各用了什么？
3. **（应用）** 看到 `kv_lora_rank=512` + `n_routed_experts=256` + `n_shared_experts=1` + `topk_method="noaux_tc"` 这组字段，是哪个系列的模型？为什么 diff 读法对它失效？
4. **（概念）** Gemma 在哪几个旋钮上和 LLaMA 明显不同（至少说两个）？为什么说它是"diff 读法收获最大"的模型？
5. **（概念）** Phi 和 Yi 这两个系列，"创新"分别在哪里？它们说明了什么趋势？

<br>

**参考答案**

1. 标准骨架 = **decoder-only + RMSNorm(pre-norm) + RoPE + GQA + SwiGLU**（LLaMA 模板）。六旋钮 = ① attention、② FFN、③ 位置编码、④ 归一化、⑤ 词表/embedding、⑥ 长上下文。（§12.1.1）
2. attention 用 **GQA**（`H_kv=8 < H`）+ **sliding window**（4096，Mistral 风格）；FFN 用 **MoE**（128 专家、top-8）。（§12.1.2）
3. **DeepSeek**（V3）。`kv_lora_rank` = MLA、256+共享专家 = DeepSeekMoE、`noaux_tc` = loss-free balance。diff 读法失效是因为 **MLA 重写了整个 attention**，和 LLaMA 差异太大，不能简单对照。（§12.5.3/12.5.4）
4. 至少两个：**sandwich norm**（子层前后各一个 norm）、**256K 词表 + tie embedding**、**GeGLU**（而非 SwiGLU）、**logit soft-cap**、**局部/全局 attention 交替**。它在多个旋钮上都偏离标准，对照 LLaMA diff 能清楚看到每个选择。（§12.6.2）
5. **Phi 重数据轻架构**（架构平平、靠合成/教科书质量数据取胜）；**Yi 刻意保持 LLaMA 兼容**（即插即用，差异在数据/训练）。趋势：**架构旋钮已被探索充分，差异化越来越转向数据和训练**。（§12.6.3/12.6.4/12.6.5）

> 第 2、3 题是本章的看家本领——拿到任意新模型的 config，十分钟看懂它"在标准模板上改了什么"。
