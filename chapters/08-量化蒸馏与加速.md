...
先回答最根本的问题：**为什么 16bit 权重能压到 4bit 还不崩**?三个原因：

- **模型过参数化**——权重里冗余巨大，推理不需要 16bit 的精度来表达每个数，4bit 的 16 个格子对大多数权重够用;
- **推理 ≠ 训练**——训练靠高精度小步累积梯度(低精度的舍入会把梯度吃掉)，推理只是一次前向;权重的微小扰动经过 residual + RMSNorm 稳着，对最终 logits 影响有限;
- **模型对噪声鲁棒**——量化误差本质是噪声，而模型本就在带噪数据上训出来，小扰动不改变预测。

所以量化的真正难点**不是**"4bit 够不够表示权重"，而是**两件事**：少数 outlier 把量化范围撑大(§8.2.3)，以及误差在 80+ 层里累积放大。下面四个方法，本质都在跟这两件事较劲。

再建立一个共识：**为什么是"权重"量化优先**。权重是固定的(offline 算好 scale，零运行时开销)，激活是动态的(要在线算 scale)。所以 **W4A16(4bit 权重 + 16bit 激活)是最甜的点**——权重压到 1/4 省显存，激活保持 16bit 避免 outlier 麻烦。下面四个方法主要解决"权重怎么压到 4bit 还不掉点"。
...
**为什么是 Hessian——关键在"优化输出误差，不是权重误差"**：朴素量化(round-to-nearest)让每个权重值各自最接近，但 GPTQ 的目标是**让这一层的输出 $Y=WX$ 改变最小**。输出对权重扰动的敏感度就是二阶导，而 $H = XX^\top$(X 是校准激活)正是这个敏感度矩阵——**某个权重方向上激活越大，那里的量化误差对输出影响越大**。所以 GPTQ 量化完一列后，用 Hessian 预测这列误差会怎么带偏输出，再把它**补偿到还没量化的列**(让后面的列主动调整、抵消已造成的输出偏差，是 OBS/OBQ 框架的闭式解)。直觉：**它容忍单个权重偏一点，只要全层输出对**——这是它在低 bit 下比 round-to-nearest 精度明显更好的根本原因。
...
**为什么"放大"就能降误差**：均匀量化的格子间距固定，所以**量化误差的绝对大小基本恒定**(约半个格子)。把一个重要权重在量化前**乘大 `s` 倍**，它相对自己的值变大，那半个格子的固定误差占它的**相对比例**就更小;量化完再除回 `s`，相对误差被一起缩小。代价是其它通道的范围被挤一点——但 AWQ 的关键发现是**只有约 1% 的"salient"通道(由激活幅度判定)真正重要**，只放大这一小撮，整体收益远大于代价。所以 AWQ 不碰激活、不用 Hessian，只做一次**离线的逐通道缩放**，又快又准。
...
> **GPTQ vs AWQ 的原理分野**(同为"降低输出误差"，下手点不同):
> - **GPTQ 在量化顺序上做文章**——逐列量化 + 把误差向后补偿，用 Hessian 算补偿量;通用稳、但要逐层跑校准，慢一些。
> - **AWQ 在量化前做一次等价缩放**——把约 1% 的 salient 通道放大、保护其相对精度;不用 Hessian、不碰激活，快，且对"激活决定重要性"这个先验拿捏得好，精度常略胜。
>
> 一句话：**GPTQ 靠"事后补偿"、AWQ 靠"事前保护"**。两者都只动权重(W4A16)，要让激活也量化(W8A8)是下一节 SmoothQuant 的活。
