...
> 一句话定位：把"让模型更小更快"的四类手段——量化、蒸馏、投机解码、稀疏化——放在一张图上对比。前三章动刀于"压一个给定模型"，蒸馏则是"换一个更小的模型"。讲清每种方案压什么、省什么、掉多少点、配哪个 kernel，让你面对一个"显存不够 / 吞吐不达标"的部署任务时，知道该上 GPTQ 还是 AWQ、该不该开 FP8、要不要蒸馏、投机解码值不值。
...
- [8.9 蒸馏：把大模型的知识压进小模型](#89-蒸馏把大模型的知识压进小模型)
- [8.10 横向对比矩阵](#810-横向对比矩阵)
- [8.11 选型决策清单](#811-选型决策清单)
- [8.12 常见坑与 FAQ](#812-常见坑与-faq)
- [自测](#自测)
- [8.13 延伸阅读](#813-延伸阅读)
...
类型 B 章节的起手式：先给一张总图，看清四类加速手段的关系和量化的核心矛盾。

### 8.2.1 四类手段，一个共同目标

![加速手段全景](../svg/19-quantization-landscape.svg)

本章的四类手段(SVG 顶部主要画了前三类)看似无关，但都在做同一件事——**省掉不必要的成本**。前三类在**给定模型上动刀**，蒸馏则**换一个更小的模型**：

| 类别 | 省什么 | 代价 | 何时介入 | 本章小节 |
|---|---|---|---|---|
| **① 量化** | 每个数的比特数 | 数值精度 | 部署期 | §8.3–8.6 |
| **② 投机解码** | 每个 token 的 forward 次数 | 额外 draft 算力 + 复杂度 | 部署期 | §8.7 |
| **③ 稀疏化** | 无效的乘加 | 可能漏掉有用计算 | 训练 / 部署 | §8.8 |
| **④ 蒸馏** | 整个模型的规模 | 要重新训练 + 能力天花板 | **训练期**（换模型） | §8.9 |

四者**正交、可叠加**：典型链路是**先蒸馏出更小的模型，再用量化 + 投机 + 稀疏把它压到极致部署**（如 DeepSeek-R1-Distill 小模型 + INT4 + 投机）。但每一种都在拿**某种余量**(精度/正确性/召回/容量)换效率——选型的本质是判断这个余量在你的任务上够不够花。
...
前面 §8.3–§8.8 的手段都在**同一个模型上动刀**——量化改比特、投机省 forward、稀疏跳计算，模型规模本身不变。蒸馏（Knowledge Distillation, KD）是本章唯一**换一个更小模型**的手段：它不压缩给定模型，而是**训练一个小模型去模仿一个大模型**——小模型叫 student、大模型叫 teacher，产出一个本就更小更快的新模型。

### 8.9.1 和前三类的根本不同：换模型，不是改模型

量化 / 投机 / 稀疏是**部署期**的事，拿到模型就能上，不动权重的“含义”。蒸馏是**训练期**的事，要花算力、数据、调参重新训一个模型出来。代价重得多，但收益也不同：它把成本一次性降到底——得到的小模型之后**还能再量化、再投机**。所以蒸馏和前三类不是竞争，是**上游**：先蒸出小模型，再用前三类把它压到极致部署。

它的吸引力在于**能力迁移**：一个小模型直接训，往往学不到大模型的某些能力；但让它去模仿大模型的输出，能“蹭”到大模型的部分本事。最典型的近例是 DeepSeek-R1——用它生成的推理过程数据，蒸馏出 R1-Distill-Qwen / R1-Distill-Llama 系列小模型，把长链推理能力迁进了 7B / 32B 量级。

### 8.9.2 核心机制：学“软标签”，不只学答案

蒸馏的关键洞察：teacher 输出的不是一个答案，是一个**概率分布**，而这个分布里藏着“暗知识”（dark knowledge）——比如分类时“猫”0.7、“狗”0.25、“汽车”0.001，这个“猫狗相近、都远离汽车”的相对关系，比一个硬标签“猫”信息量大得多。student 学这个**软分布**，比只学硬标签学得快、学得好。

做法是用**温度** $T$ 把分布软化（$T>1$ 让分布更平、暴露更多相对信息），再让 student 的软分布去逼近 teacher 的：

$$\mathcal{L} = \alpha\, T^2\, \mathrm{KL}\!\big(p_S^{(T)} \,\|\, p_T^{(T)}\big) \;+\; (1-\alpha)\,\mathrm{CE}(p_S,\, y), \qquad p^{(T)} = \mathrm{softmax}(\text{logits}/T)$$

第一项让 student 对齐 teacher 的软分布（$T^2$ 补偿软化带来的梯度缩放），第二项是常规的真实标签交叉熵，$\alpha$ 调两者权重。代码骨架就是这个公式的直译：

```python
import torch.nn.functional as F
def kd_loss(student_logits, teacher_logits, labels, T=2.0, alpha=0.9):
    s = F.log_softmax(student_logits / T, dim=-1)
    t = F.softmax(teacher_logits / T, dim=-1)        # teacher 不回传梯度
    kl = F.kl_div(s, t, reduction="batchmean") * (T * T)
    ce = F.cross_entropy(student_logits, labels)
    return alpha * kl + (1 - alpha) * ce
```

### 8.9.3 三种形态：序列级 / logit 级 / 特征级

LLM 语境下，蒸馏按“对齐 teacher 的什么”分三档，约束递增、要求也递增：

| 形态 | student 对齐 teacher 的 | 要求 | 典型用法 |
|---|---|---|---|
| **序列级 / 黑盒**（数据蒸馏） | 只对齐**输出文本** | 只需 teacher 能生成数据，连 logits 都不用 | 用强模型造 (prompt, response) 数据，student 在上面 SFT；最简单最常用（R1-Distill 即此路） |
| **logit 级 / 白盒** | 每步的 **logits 分布** | teacher 同 tokenizer、可取 logits、要在线前向 | 信息最密，上面的 `kd_loss` 即此 |
| **特征级** | 还对齐**中间层 hidden / attention** | teacher 与 student 结构可对应 | 约束最强，适合同架构缩规模 |

一个重要的工程取向是 **on-policy 蒸馏**（如 GKD）：让 student **自己生成**序列、再让 teacher 对这些序列打分，而不是只喂 teacher 生成的数据。这样训练分布和推理分布一致，避免“teacher 的数据里没有 student 实际会犯的错”导致的暴露偏差，效果通常更好，代价是训练时要 teacher 在线参与。

### 8.9.4 适用与限制

- **适合**：想要一个**永久更小更快**的模型（部署成本一次性降到底）、把强模型的特定能力（推理、领域知识）迁到小模型、或在小模型上复刻大模型的风格 / 对齐。
- **限制**：① **要训练**——算力、数据、调参成本远高于量化这种即插即用的手段；② **有能力天花板**——student 容量有限，蒸不出 teacher 的全部本事，越想压小、损失越大；③ **序列级蒸馏强依赖 teacher 输出质量**，teacher 错了 student 跟着错。

> 一句话：**蒸馏是本章唯一“换模型”而非“改模型”的手段——训练一个小 student 去模仿大 teacher，学的是软分布里的“暗知识”。** 它和量化 / 投机 / 稀疏正交且互补：蒸馏产出更小的新模型，再用那三类把它压到极致部署（DeepSeek-R1-Distill 就是这条链的范例）。代价是要重新训练、且有能力天花板。
...
| **蒸馏(KD)** | 整个模型规模 | 取决于 student 大小 | 同比例提升 | 有能力天花板 | **要重新训练** | §8.9 |
...
1. **先 profile 定瓶颈**(§8.10.2)——显存 / decode 带宽 / prefill 算力 / 长 ctx KV，瓶颈不同手段完全不同;
...
10. **多种量化叠加后精度崩**：精度损失会累积(§8.10.3)。逐个加、逐个验证，别一次全开。
...
5. 错在**假设各自的精度损失独立可加**。组合量化的掉点会累积，必须在自己的 eval 集上验证组合效果，逐个加、逐个测。（§8.10.3、§8.12 第 10 条）
