...
6. **被术语劝退**：回到 P.1 的"大图"（P.1.6），或查全书[术语表 `glossary.md`](../glossary.md)——每个词一句话解释 + 去哪一章看透。任何术语，先问它落在大图哪一步，再去对应阶段看细节。**先有钩子，再填细节。**
...
不翻书试着答，再看参考答案。答得上来，说明你"上车"了。

1. **（概念）** 用一句话说清"训练"和"推理"的区别。你平时和 AI 聊天，用的是哪个？
2. **（口算）** 一个 7B 模型，用 BF16（每个参数 2 字节）存，光参数大约要多少显存？
3. **（概念）** "今天天气怎样" 在模型眼里不是 6 个字，而是什么？为什么全书的"长度/速度/显存"都按它算，而不按字数？
4. **（应用）** 推理分 prefill 和 decode 两步：哪一步决定"第一个字多久蹦出来"，哪一步决定"后面每个字多快"？哪个偏吃算力、哪个偏吃显存带宽？
5. **（概念）** KV cache 是什么的"短期记忆"？为什么"对话越长越占显存"？

<br>

**参考答案**

1. **训练**是拧模型的参数（旋钮）把它造出来，参数一直在变、极贵；**推理**是用造好的模型给输入出输出，参数固定不动。日常聊天用的是**推理**。（P.1.3）
2. **约 14 GB**：7B = 70 亿参数 × 2 字节 = 140 亿字节 ≈ 14 GB。（P.1.1）—— 这还只是参数，加上 KV cache 等会更多。
3. 是一串 **token**（切成小块再编号的数字），不是字。因为 **token 是模型处理文字的最小单位**，一个汉字可能对应 1–2 个 token，按字算会对不上。（P.1.2）
4. **prefill** 决定第一个字的延迟（TTFT），偏**吃算力**；**decode** 决定后续每个字的速度（TPOT），偏**吃显存带宽**。（P.1.4）
5. KV cache 是模型对"**前面已经读过的每个 token**"的短期记忆，存下来避免重算。对话越长，要记的 token 越多，KV cache 越大，**越占显存**——这是全书的"显存第一矛盾"。（P.1.5）

> 5 题都答得上 → 你已建立全书需要的"大图"，可以正式去[阶段 0](00-prereq-hardware.md)。答不上某题 → 回对应的 P.1.x 再看一眼，那里有大白话解释。
