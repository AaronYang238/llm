...
回阶段 1 §1.2.3：RoPE 把位置 m 编码成旋转角度 $m\theta_i$，其中 $\theta_i = \text{base}^{-2i/d}$。问题在于——**模型只在训练长度内见过这些旋转角度**。推理超出训练长度时，出现两类没见过的情况：
...
**更多更小的专家 → 指数级更多的组合 → 每个 token 能得到更精准的"技能配方"**。同时，把专家切小后，每个专家参数少，激活 8 个的总算力反而可控（256 选 8 ≈ 激活 3.1%，含 1 个共享专家约 3.5%，回阶段 8 §8.8.2）。这是"用组合灵活性换表达力，用稀疏激活控算力"。
