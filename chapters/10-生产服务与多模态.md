...
1. **（概念）** 为什么所有开源推理引擎都要做"OpenAI 兼容 API"？streaming（SSE）为什么直接关系到用户体验？
2. **（概念）** 结构化输出是怎么保证模型"只能生成合法 JSON"的？它在采样管线的哪一步动手？为什么说它和 tool use 是同一套机制？
3. **（应用）** 几十个业务各有一个 LoRA 微调，为什么能在一个服务里同时跑、且几乎不增显存？难点在哪、怎么解决？
4. **（概念）** 一张图片是怎么变成 LLM 能吃的 token 的？为什么说"没有专门的多模态推理引擎"？
5. **（判读）** 多模态/视频推理特别容易 OOM，根因是什么？这把问题引向了前面哪一章的技术？

<br>

**参考答案**

1. 生态网络效应——所有框架/SDK 都按 OpenAI 格式来，兼容它才能即插即用。streaming 让**第一个 chunk 到达 = TTFT**、chunk 间隔 = TPOT，直接成为用户感知的延迟。（§10.2.1/10.2.3）
2. 在 **softmax 之前对 logits 做 mask**，把所有"会导致不合法"的 token 概率设为 −∞，用 FSM/CFG 状态机驱动。tool use 的 `arguments` 必须是合法 JSON，底层就是结构化输出。（§10.3.1/10.3.2、§10.2.4）
3. 省显存来自 LoRA 的**低秩本质**（基座共享一份、每个 LoRA 只几十 MB 增量）。难点是一个 batch 里混了不同 LoRA；用**分段 kernel**（SGMV/Punica）把"异构 LoRA"编码成索引，一个 kernel 处理混合 batch。（§10.4）
4. 图像经 **vision encoder（ViT）→ projector** 投影到 LLM 词向量空间，变成"image token"，和文本 token 拼接喂同一个 LLM。所以多模态 = 视觉前处理 + 复用纯文本 LLM 后半段。（§10.5.1）
5. 图像/视频 **token 数暴涨**（1 张图 256+、视频上万），prefill 和 KV cache 成本远超纯文本。这把多模态变成"超长 token 序列"推理，引向**阶段 9 的长上下文技术**（MLA 压 KV、序列并行）。（§10.5.3）

> 第 2、3 题是生产服务两个高频能力（可控输出、多租户 LoRA）——能讲清机制，说明 A 类前半读透了。
