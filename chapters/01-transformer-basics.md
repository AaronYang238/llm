...
$\text{scores}[i,j]$ 衡量"token $i$ 该多关注 token $j$"——两个向量方向越一致，内积越大，越该关注（分母 $\sqrt{d}$ 的作用见 §1.2.1.3）。
...
**多头注意力**（Multi-Head Attention）的做法：把表示维度 $D$ 切成 $H$ 个 head、每个 head $d = D/H$ 维，**各自独立**做一遍上面的 Q/K/V attention，让每个 head 学不同的关注模式，最后把 $H$ 个 head 的输出拼回 $D$ 维，再过 $W_O$ 融合。shape 流转：
...
2. **KV cache**——§1.2.2 的公式，随 $B \times S$ **线性增长**。上图就是这块的故事：LLaMA-2-7B（MHA，1 MB/token）在 $B=8$、$S=8\text{K}$ 时 KV 就 64 GB，逼近 80 GB 卡的上限；再大就 OOM。**这块是推理 OOM 的头号元凶**，也是 GQA/MLA（§1.2.2）拼命压它的原因。
